...
exports.ensureSeededSchemes = ensureSeededSchemes;
const GovtScheme_1 = require("../models/GovtScheme");
const logger_1 = require("./logger");
async function ensureSeededSchemes(force = false) {
    try {
        ...
        if (force) {
            await GovtScheme_1.GovtScheme.deleteMany({});
            logger_1.logger.info('Cleared existing schemes for fresh re-seeding...');
        }
        logger_1.logger.info('Seeding 10 Real Government Schemes for Rajasthan & Central India...');
        const schemes = [
            {
        ...
        await GovtScheme_1.GovtScheme.insertMany(schemes);
        logger_1.logger.info(`Successfully seeded ${schemes.length} real government schemes.`);
    }
    catch (err) {
        logger_1.logger.error('Failed to seed schemes:', err);
    }
}
//# sourceMappingURL=seedSchemes.js.map