...
exports.buildPageContextBlock = buildPageContextBlock;
exports.buildMismatchWarning = buildMismatchWarning;
...
function buildPageContextBlock(pageData, userMessage) {
    const ctx = pageData.pageContext || 'ui';
    const instruction = PAGE_INSTRUCTIONS[ctx] || PAGE_INSTRUCTIONS.ui;
    // Detect intent from user message
    const intent = (0, intentEngine_1.detectIntent)(userMessage);
    const intentName = (0, intentEngine_1.intentLabel)(intent);
 ...
 */
function buildMismatchWarning(pageCtx, intent) {
    // These intents are always allowed on any page
    const universalIntents = ['general', 'navigation', 'voice_command'];
    ...
    if (!expectedCtx || expectedCtx === pageCtx || pageCtx === 'dashboard' || pageCtx === 'ui')
        return '';
    return `\n\nCONTEXT MISMATCH WARNING: The user is asking about "${(0, intentEngine_1.intentLabel)(intent)}" but is currently on the "${pageCtx}" page. Answer the question but gently remind the farmer they can navigate to the ${(0, intentEngine_1.intentLabel)(intent)} page for full details. Do NOT ignore the current page context entirely.`;
}
