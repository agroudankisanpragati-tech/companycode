...
exports.generateDiseaseResponse = generateDiseaseResponse;
exports.generateLowConfidencePrompt = generateLowConfidencePrompt;
...
function severityEmoji(severity) {
    ...
    if (s.includes('very high') || s.includes('critical'))
        return '🔴';
    if (s.includes('high'))
        return '🟠';
 ...
 */
function generateDiseaseResponse(kbResult, yoloConf, // YOLO confidence 0–100
cropName, hasImage) {
    ...
    // Cause
    if (d.cause)
        lines.push(section('🔬', H.cause, d.cause));
    // Symptoms
    if (d.symptoms)
        lines.push(section('👁️', H.symptoms, d.symptoms));
    // Organic treatment
    if (d.organicSolution)
        lines.push(section('🌿', H.organic, d.organicSolution));
    // Chemical treatment
    if (d.chemicalSolution)
        lines.push(section('💊', H.chemical, d.chemicalSolution));
    // Prevention
    if (d.prevention)
        lines.push(section('🛡️', H.prevention, d.prevention));
    // Fertilizer advice
    if (d.fertilizerAdvice)
        lines.push(section('🌱', H.fertilizer, d.fertilizerAdvice));
    // Irrigation advice
    if (d.irrigationAdvice)
        lines.push(section('💧', H.irrigation, d.irrigationAdvice));
    // Dos and Don'ts
    if (d.dos)
        lines.push(section('✅', H.dos, Array.isArray(d.dos) ? bullet(d.dos) : d.dos));
    if (d.donts)
        lines.push(section('❌', H.donts, Array.isArray(d.donts) ? bullet(d.donts) : d.donts));
    // Recovery tips
    if (d.recoveryTips)
        lines.push(section('💪', H.recovery, d.recoveryTips));
    // Warnings
    if (d.warnings)
        lines.push(section('⚠️', H.warnings, Array.isArray(d.warnings) ? bullet(d.warnings) : d.warnings));
    // Next steps
    if (d.nextSteps) {
    ...
    if (d.cropName)
        hindiLines.push(`🌾 फसल: ${d.cropName}`);
    if (yoloConf !== undefined)
        hindiLines.push(`\n${H.confidence}: ${confidenceBar(confScore)}`);
    if (d.severity)
        hindiLines.push(`${sevEmoji} ${H.severity}: ${d.severity}`);
    if (d.cause)
        hindiLines.push(section('🔬', H.cause, d.cause));
    if (d.symptoms)
        hindiLines.push(section('👁️', H.symptoms, d.symptoms));
    if (d.organicSolution)
        hindiLines.push(section('🌿', H.organic, d.organicSolution));
    if (d.chemicalSolution)
        hindiLines.push(section('💊', H.chemical, d.chemicalSolution));
    if (d.prevention)
        hindiLines.push(section('🛡️', H.prevention, d.prevention));
    if (d.fertilizerAdvice)
        hindiLines.push(section('🌱', H.fertilizer, d.fertilizerAdvice));
    if (d.irrigationAdvice)
        hindiLines.push(section('💧', H.irrigation, d.irrigationAdvice));
    if (d.dos)
        hindiLines.push(section('✅', H.dos, Array.isArray(d.dos) ? bullet(d.dos) : d.dos));
    if (d.donts)
        hindiLines.push(section('❌', H.donts, Array.isArray(d.donts) ? bullet(d.donts) : d.donts));
    if (d.recoveryTips)
        hindiLines.push(section('💪', H.recovery, d.recoveryTips));
    if (d.warnings)
        hindiLines.push(section('⚠️', H.warnings, Array.isArray(d.warnings) ? bullet(d.warnings) : d.warnings));
    hindiLines.push(`\n**📋 ${H.nextSteps}**`);
...
function buildAskForImageResponse(cropName) {
...
function buildAskForImageResponseHindi(cropName) {
...
function buildLowConfidenceResponse(cropName, conf) {
...
function buildLowConfidenceResponseHindi(cropName, conf) {
...
function buildNoMatchResponse(cropName) {
...
function buildNoMatchResponseHindi(cropName) {
 ...
 */
function generateLowConfidencePrompt(domain, missing) {
    const english = [
