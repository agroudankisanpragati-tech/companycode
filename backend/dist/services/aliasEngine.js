...
exports.processUnknownWord = processUnknownWord;
exports.processBatchUnknownWords = processBatchUnknownWords;
exports.extractCandidateWords = extractCandidateWords;
 ...
 */
async function processUnknownWord(rawInput, pageContext, detectedLang, suggestedEnglish) {
    const normalized = (0, languageDictionaryService_1.normalizeKey)(rawInput);
    if (!normalized || normalized.length < 2) {
        return { action: 'skip_noise', normalizedInput: normalized };
    }
    // Step 1: Check exact match in approved dictionary
    const exact = await LanguageDictionary_1.LanguageDictionary.findOne({
        approved: true,
    ...
    const allEntries = await LanguageDictionary_1.LanguageDictionary.find({ approved: true })
    ...
    if (bestScore >= MERGE_THRESHOLD && bestEntry) {
        ...
        await upsertReviewQueue(rawInput, normalized, pageContext, detectedLang, suggestedEnglish, bestEntry._id.toString(), bestScore);
        return {
...
// ─── Queue upsert helper ──────────────────────────────────────────────────────
async function upsertReviewQueue(rawInput, normalizedKey, pageContext, detectedLang, suggestedEnglish, suggestedMergeTargetId, similarity) {
    try {
        await DictionaryReviewQueue_1.DictionaryReviewQueue.updateOne({ normalizedKey, status: 'pending' }, {
            $setOnInsert: {
                rawInput,
                normalizedKey,
                pageContext,
 ...
 */
async function processBatchUnknownWords(words, pageContext, detectedLang) {
    return Promise.all(words.map(w => processUnknownWord(w, pageContext, detectedLang)));
}
/**
 * Extract candidate unknown words from a message.
 * Filters out common stop words and very short tokens.
 */
function extractCandidateWords(message) {
...
//# sourceMappingURL=aliasEngine.js.map