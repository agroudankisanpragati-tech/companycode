...
exports.composeLocalResponse = composeLocalResponse;
exports.buildNotFoundResponse = buildNotFoundResponse;
// ─── Language name map ────────────────────────────────────────────────────────
const LANG_NAMES = {
...
function composeDiseaseResponse(results) {
    ...
    if (d.structuredResponse?.english)
        return d.structuredResponse.english;
    // Need at least disease name or symptoms to give a useful answer
    if (!d.diseaseName && !d.symptoms && !d.organicSolution)
        return null;
    const lines = [];
    if (d.diseaseName)
        lines.push(`🌿 **Disease/Pest:** ${d.diseaseName} on ${d.cropName || 'your crop'}`);
    if (d.symptoms)
        lines.push(section(`${H.symptoms}`, d.symptoms));
    ...
    if (d.prevention)
        lines.push(section(`${H.prevention}`, d.prevention));
    if (d.dos)
        lines.push(section('✅ Do', d.dos));
    if (d.donts)
        lines.push(section('❌ Don\'t', d.donts));
    if (d.recoveryTips)
        lines.push(section('💊 Recovery Tips', d.recoveryTips));
    return lines.filter(Boolean).join('\n');
}
function composeCropResponse(results) {
    const agent = results.find(r => r.agent === 'CropAgent' && r.success && r.data);
    if (!agent?.data || Object.keys(agent.data).length === 0)
        return null;
    const d = agent.data;
    if (!d.cropName && !d.lastRequestSoilType)
        return null;
    const lines = [];
    if (d.cropName) {
        ...
        if (d.whySuitable)
            lines.push(section('Why Suitable', d.whySuitable));
        if (d.cultivationGuide)
            lines.push(section('Cultivation Guide', d.cultivationGuide));
        if (d.fertilizerPlan)
            lines.push(section(`${H.fertilizer} Plan`, d.fertilizerPlan));
    }
...
function composeSoilResponse(results) {
    const agent = results.find(r => r.agent === 'SoilAgent' && r.success && r.data);
    if (!agent?.data || Object.keys(agent.data).length === 0)
        return null;
    const d = agent.data;
    if (!d.healthScore && !d.ph && !d.nitrogen)
        return null;
    const lines = [];
    ...
    if (d.deficiencies?.length) {
        lines.push(section(`⚠️ ${H.deficiency}`, bullet(d.deficiencies.map((def) => def.nutrient || def))));
    }
    ...
    if (d.reasoning)
        lines.push(section('Analysis', d.reasoning));
    return lines.filter(Boolean).join('\n');
}
function composeWeatherResponse(results) {
    const agent = results.find(r => r.agent === 'WeatherAgent' && r.success && r.data);
    if (!agent?.data || Object.keys(agent.data).length === 0)
        return null;
    const d = agent.data;
    if (!d.condition && !d.temp)
        return null;
    const lines = [];
    ...
    if (d.forecast?.length) {
        const forecastLines = d.forecast.slice(0, 3).map((f) => `  ${f.date || ''}: ${f.condition || ''}, ${f.maxTemp || ''}°C / ${f.minTemp || ''}°C, Rain: ${f.rainChance || 0}%`);
        lines.push(section('3-Day Forecast', forecastLines.join('\n')));
    }
    // Farming advisory based on weather
    const advice = [];
    if (d.humidity > 80)
        advice.push('High humidity — watch for fungal diseases on crops.');
    if (d.temp > 40)
        advice.push('Extreme heat — irrigate in early morning or evening.');
    if (d.temp < 10)
        advice.push('Cold weather — protect sensitive crops from frost.');
    if (d.forecast?.some((f) => f.rainChance > 60))
        advice.push('Rain expected — delay pesticide application.');
    if (advice.length)
        lines.push(section('🌾 Farming Advisory', bullet(advice)));
    return lines.filter(Boolean).join('\n');
}
function composeMarketResponse(results) {
    const agent = results.find(r => r.agent === 'MarketAgent' && r.success && r.data);
    if (!agent?.data || Object.keys(agent.data).length === 0)
        return null;
    const d = agent.data;
    if (!d.commodity && !d.modalPrice)
        return null;
    const lines = [];
...
function composeGovernmentResponse(results) {
    const agent = results.find(r => r.agent === 'GovernmentAgent' && r.success && r.data);
    if (!agent?.data || Object.keys(agent.data).length === 0)
        return null;
    const d = agent.data;
    const schemes = d.schemes || (d.title ? [d] : []);
    if (schemes.length === 0)
        return null;
    const lines = [];
    ...
    for (const s of schemes.slice(0, 2)) {
        ...
        if (s.benefits?.length)
            lines.push(section(`✅ ${H.benefits}`, bullet(s.benefits)));
        if (s.eligibility)
            lines.push(section(`📌 ${H.eligibility}`, s.eligibility));
        if (s.applicationProcess)
            lines.push(section(`📝 ${H.apply}`, s.applicationProcess));
        if (s.applicationLink)
            lines.push(`🔗 Apply: ${s.applicationLink}`);
    }
    return lines.filter(Boolean).join('\n');
}
function composeKVKResponse(results) {
    const agent = results.find(r => r.agent === 'KVKAgent' && r.success && r.data);
    if (!agent?.data || Object.keys(agent.data).length === 0)
        return null;
    const d = agent.data;
    const kvks = d.kvks || (d.name ? [d] : []);
    if (kvks.length === 0)
        return null;
    const lines = [];
    lines.push(`🏫 **${H.kvk}**`);
    for (const k of kvks.slice(0, 2)) {
        ...
        if (k.services?.length)
            lines.push(section('Services', bullet(k.services)));
    }
    return lines.filter(Boolean).join('\n');
}
function composeIrrigationResponse(results) {
    const agent = results.find(r => r.agent === 'IrrigationAgent' && r.success && r.data);
    if (!agent?.data || Object.keys(agent.data).length === 0)
        return null;
    const d = agent.data;
    if (!d.irrigationMethod && !d.soilMoisturePercent && !d.recommendation)
        return null;
    const lines = [];
    if (d.cropName)
        lines.push(`💧 **Irrigation — ${d.cropName}**`);
    else
        lines.push('💧 **Irrigation Advisory**');
...
function composeFarmDiaryResponse(results) {
    ...
    if (d.todayTasks?.length) {
        const taskLines = d.todayTasks.map((t) => typeof t === 'string' ? t : `${t.title}${t.description ? ': ' + t.description : ''}`);
...
function composeFertilizerResponse(results) {
    const agent = results.find(r => r.agent === 'FertilizerAgent' && r.success && r.data);
    if (!agent?.data || Object.keys(agent.data).length === 0)
        return null;
    const d = agent.data;
    if (!d.products?.length && !d.soilContext)
        return null;
    const lines = [];
    ...
    if (d.products?.length) {
        const productLines = d.products.slice(0, 3).map((p) => `${p.name} (${p.type || ''}) — ${p.nutrientContent || ''} — ₹${p.price || 'N/A'}`);
...
function composeAllAvailableData(results) {
    const parts = [];
    const agents = [];
    const tryAdd = (text, agent) => {
        if (text?.trim()) {
            parts.push(text);
            agents.push(agent);
        }
...
function toHindi(english, langCode) {
    // If user is already in Hindi/Devanagari, return as-is
    if (langCode === 'hi' || langCode === 'raj' || langCode === 'mai')
        return english;
 ...
 */
function composeLocalResponse(intent, results, langCode, userMessage) {
    ...
    switch (intent) {
        case 'disease':
            ...
            if (english)
                agentsUsed.push('DiseaseAgent');
            break;
        case 'crop':
            english = composeCropResponse(results);
            if (english)
                agentsUsed.push('CropAgent');
            if (english) {
                const fertExt = composeFertilizerResponse(results);
                if (fertExt) {
                    english += '\n' + fertExt;
                    agentsUsed.push('FertilizerAgent');
                }
            }
            break;
        case 'soil': {
    ...
    const confidence = agentsUsed.length >= 2 ? 'high' :
 ...
 */
function buildNotFoundResponse(langCode) {
...
//# sourceMappingURL=localResponseComposer.js.map