...
exports.runCropAgent = runCropAgent;
...
async function runCropAgent(ctx) {
    try {
        ...
        const lastRequest = await FarmerCropRequest_1.FarmerCropRequest.findOne({ farmerId: userId })
            .sort({ createdAt: -1 })
            .lean();
        if (lastRequest) {
            return {
...
function extractSeasonFromMessage(msg) {
    const lower = msg.toLowerCase();
    if (/kharif|monsoon|rainy|sawan/.test(lower))
        return 'Kharif';
    if (/rabi|winter|sardi/.test(lower))
        return 'Rabi';
    if (/zaid|summer|garmi/.test(lower))
        return 'Zaid';
    return '';
}
//# sourceMappingURL=cropAgent.js.map