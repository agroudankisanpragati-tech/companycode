...
const SoilStandard_1 = require("../models/SoilStandard");
const soilAIService_1 = require("../services/soilAIService");
const translationService_1 = require("../services/translationService");
const router = express_1.default.Router();
// Configure multer for soil report uploads
const uploadDir = path_1.default.join(process.cwd(), 'uploads', 'soil');
...
const upload = (0, multer_1.default)({
    ...
    fileFilter: (_req, file, cb) => {
        ...
        if (allowed.test(path_1.default.extname(file.originalname)) && allowed.test(file.mimetype.split('/')[1])) {
            cb(null, true);
        }
        else {
...
async function ensureSoilStandards() {
    const count = await SoilStandard_1.SoilStandard.countDocuments();
    if (count === 0) {
        await SoilStandard_1.SoilStandard.insertMany(SoilStandard_1.SOIL_STANDARDS_DATA);
    ...
    if (ext === '.pdf') {
        try {
            ...
            for (const block of matches) {
                ...
                text += strMatches.map((s) => s.slice(1, -1)).join(' ') + ' ';
            }
...
async function getMatchingStandard(soilType) {
    if (soilType) {
        const normalized = soilType.trim().toLowerCase();
        const standards = await SoilStandard_1.SoilStandard.find();
    ...
    return await SoilStandard_1.SoilStandard.findOne({ soilType: 'Alluvial' });
}
// POST /api/soil/upload — Upload soil report file and run full AI analysis
router.post('/upload', auth_1.authenticate, upload.single('report'), async (req, res) => {
    try {
        ...
        const defaultStandard = await SoilStandard_1.SoilStandard.findOne({ soilType: 'Alluvial' });
        if (!defaultStandard)
            return res.status(500).json({ error: 'Soil standards not initialized' });
        // Step 3: AI analysis — extract data + generate recommendations
        const aiResult = await (0, soilAIService_1.extractAndAnalyzeSoilWithAI)(ocrText, defaultStandard);
        ...
        const { score, status } = (0, soilAIService_1.calculateSoilHealthScore)(aiResult, matchedStandard);
        const benchmarkComparison = (0, soilAIService_1.buildBenchmarkComparison)(aiResult, matchedStandard);
        const deficiencies = (0, soilAIService_1.detectDeficiencies)(aiResult, matchedStandard);
        // Step 6: Save to database
        const soilReport = await SoilReport_1.SoilReport.create({
            farmerId,
            reportUrl,
            uploadDate: new Date(),
...
router.post('/analyze', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const standard = await getMatchingStandard(soilType) || await SoilStandard_1.SoilStandard.findOne({ soilType: 'Alluvial' });
        if (!standard)
            return res.status(500).json({ error: 'Soil standards not initialized' });
        const { score, status } = (0, soilAIService_1.calculateSoilHealthScore)(soilData, standard);
        const benchmarkComparison = (0, soilAIService_1.buildBenchmarkComparison)(soilData, standard);
        const deficiencies = (0, soilAIService_1.detectDeficiencies)(soilData, standard);
        const aiResult = await (0, soilAIService_1.generateAIAnalysisFromData)(soilData, standard);
        const soilReport = await SoilReport_1.SoilReport.create({
            farmerId,
            uploadDate: new Date(),
...
router.get('/history', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const reports = await SoilReport_1.SoilReport.find({ farmerId })
            .sort({ createdAt: -1 })
            .skip((page - 1) * limit)
            .limit(limit)
            .select('uploadDate soilType soilHealthScore soilHealthStatus reportUrl createdAt');
        const total = await SoilReport_1.SoilReport.countDocuments({ farmerId });
...
router.get('/crops/:id', auth_1.authenticate, async (req, res) => {
    try {
        const report = await SoilReport_1.SoilReport.findById(req.params.id).select('cropRecommendations soilType soilHealthScore farmerId');
        if (!report)
...
router.post('/translate', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const report = await SoilReport_1.SoilReport.findById(recordId);
        if (!report)
            return res.status(404).json({ error: 'Report not found' });
        if (report.farmerId.toString() !== req.user.userId)
            return res.status(403).json({ error: 'Access denied' });
        // Check if translation already exists
        const existing = report.translations?.get?.(language) ?? report.translations?.[language];
        if (existing) {
        ...
        const translated = await (0, translationService_1.translateNestedObject)(enData, language);
        // Permanently save to DB
        await SoilReport_1.SoilReport.findByIdAndUpdate(recordId, {
...
router.delete('/:id', auth_1.authenticate, async (req, res) => {
    try {
        const report = await SoilReport_1.SoilReport.findById(req.params.id);
        if (!report)
            return res.status(404).json({ error: 'Report not found' });
        if (report.farmerId.toString() !== req.user.userId)
            return res.status(403).json({ error: 'Access denied' });
        ...
        if (report.reportUrl) {
            const filePath = path_1.default.join(process.cwd(), report.reportUrl);
            if (fs_1.default.existsSync(filePath))
                fs_1.default.unlinkSync(filePath);
        }
        await SoilReport_1.SoilReport.findByIdAndDelete(req.params.id);
