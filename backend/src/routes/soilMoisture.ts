...
function deriveMoisture(rainfallMm: number, humidity: number, state: string): number {
  ...
  if (aridStates.some((s) => normalized.includes(s))) base -= 8;
  if (wetStates.some((s) => normalized.includes(s))) base += 8;

  return Math.round(Math.max(5, Math.min(95, base)));
}

function getMoistureStatus(pct: number): string {
...
async function fetchRainfallFromGov(state: string, district: string): Promise<{ rainfallMm: number; humidity: number }> {
  if (!getDataGovKey()) return { rainfallMm: 0, humidity: 60 };
  ...
  try {
    ...
    if (!res.ok) return { rainfallMm: 0, humidity: 60 };
...
async function fetchWeatherHumidity(state: string, district: string): Promise<{ humidity: number; rainfallMm: number }> {
  try {
    const query = `${district}, ${state}, India`;
    const baseUrl = process.env.WEATHER_API_BASE_URL || 'http://localhost:4000';
    const res = await fetch(
      `${baseUrl}/api/weather?location=${encodeURIComponent(query)}`,
      { signal: AbortSignal.timeout(6000) }
    );
    if (!res.ok) return { humidity: 60, rainfallMm: 0 };
...
router.get('/', authenticate, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    // Check cache first
    const cached = await SoilMoisture.findOne({ farmerId });
    if (cached) {
    ...
    const record = await SoilMoisture.findOneAndUpdate(
      { farmerId },
      { farmerId, state, district, moisturePercentage, moistureStatus, rainfallMm, humidity, lastUpdated: new Date() },
...
router.post('/location', authenticate, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const farmerId = req.user!.userId;
    ...
    const record = await SoilMoisture.findOneAndUpdate(
      { farmerId },
      { farmerId, state: state.trim(), district: district.trim(), moisturePercentage, moistureStatus, rainfallMm, humidity, lastUpdated: new Date() },
      { upsert: true, new: true }
    );
