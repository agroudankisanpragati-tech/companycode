...
function extractLastDisease(history: IConversationTurn[]): string {
...
function extractLastCrop(history: IConversationTurn[]): string {
  const cropList = [
...
function extractLastScheme(history: IConversationTurn[]): string {
...
function extractLastCommodity(history: IConversationTurn[]): string {
  const commodities = [
...
function extractLastLocation(history: IConversationTurn[]): string {
...
export function resolveContextReferences(
  ...
  const refs: ResolvedContext['resolvedRefs'] = {};
  const contextParts: string[] = [];

  if (diseaseName) { refs.diseaseName = diseaseName; contextParts.push(`disease: ${diseaseName}`); }
  if (cropName)    { refs.cropName    = cropName;    contextParts.push(`crop: ${cropName}`); }
  if (schemeName)  { refs.schemeName  = schemeName;  contextParts.push(`scheme: ${schemeName}`); }
  if (commodity)   { refs.commodity   = commodity;   contextParts.push(`commodity: ${commodity}`); }
  if (location)    { refs.location    = location;    contextParts.push(`location: ${location}`); }
 ...
 */
export function buildContextSummary(history: IConversationTurn[]): string {
