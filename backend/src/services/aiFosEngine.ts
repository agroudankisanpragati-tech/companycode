...
const CROP_TEMPLATES: Record<string, { duration: number; stages: StageInfo[]; tasks: TaskTemplate[] }> = {
  wheat: {
    duration: 120,
    stages: [
...
function getTemplate(cropName: string) {
...
// ── Stage resolver ───────────────────────────────────────────────────────────
export function resolveStage(stages: StageInfo[], dayAge: number): string {
  for (const s of stages) {
    if (dayAge >= s.startDay && dayAge <= s.endDay) return s.name;
  }
  if (dayAge < stages[0].startDay) return stages[0].name;
...
async function generateLifecycleWithAI(
  cropName: string,
  duration: number,
  state: string,
  district: string
...
Generate 10-15 key tasks covering: land prep, sowing, irrigations, fertilizers, pest control, harvest.`;
  ...
  return (parsed.tasks || []) as TaskTemplate[];
}
...
export async function getCropLifecycle(
  ...
  state: string,
  district: string
): Promise<{ duration: number; stages: StageInfo[]; tasks: TaskTemplate[] }> {
...
export async function generateDailyRecommendation(context: {
  cropName: string;
  dayAge: number;
  stage: string;
  moisture?: number;
  ...
  const apiKey = process.env.OPENAI_API_KEY;
  if (!apiKey) return `Your ${context.cropName} is at Day ${context.dayAge} (${context.stage} stage). Follow the scheduled tasks for today.`;
