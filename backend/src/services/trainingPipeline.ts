...
import { TrainingDataset, ITranscriptEntry, DatasetStatus } from '../models/TrainingDataset';
 ...
 */
export async function importDataset(input: DatasetImportInput): Promise<typeof TrainingDataset.prototype> {
  // Find latest version for this name+lang
  const latest = await TrainingDataset.findOne({ name: input.name, langCode: input.langCode })
    .sort({ version: -1 })
    .lean();

  const version = latest ? bumpVersion(latest.version) : '1.0.0';

  const transcripts: ITranscriptEntry[] = input.transcripts.map(t => ({
...
export async function validateDataset(datasetId: string): Promise<ValidationResult> {
  ...
  await TrainingDataset.findByIdAndUpdate(datasetId, { status: 'validating' });
  ...
  const finalStatus: DatasetStatus = errors.length === 0 ? 'validated' : 'imported';

  await TrainingDataset.findByIdAndUpdate(datasetId, {
 ...
 */
export async function approveDataset(datasetId: string, adminUserId: string): Promise<void> {
  const dataset = await TrainingDataset.findById(datasetId);
  if (!dataset) throw new Error('Dataset not found');
 ...
 */
export async function rejectDataset(datasetId: string, adminUserId: string, reason?: string): Promise<void> {
  const dataset = await TrainingDataset.findById(datasetId);
  if (!dataset) throw new Error('Dataset not found');
...
export async function listDatasets(filters: {
  ...
  const filter: any = {};
  if (langCode) filter.langCode = langCode;
  if (status)   filter.status   = status;

  const [data, total] = await Promise.all([
    TrainingDataset.find(filter)
      .select('-transcripts')  // exclude large transcript array from list
      .sort({ createdAt: -1 })
      .skip((page - 1) * limit)
      .limit(limit)
      .lean(),
    TrainingDataset.countDocuments(filter),
  ]);
 ...
 */
export async function getApprovedDatasets(langCode: string): Promise<any[]> {
  return TrainingDataset.find({ langCode, status: 'approved' })
 ...
 */
export async function syncFarmerVoiceDatasets(): Promise<{ synced: number }> {
  ...
  for (const memory of memories) {
    for (const ref of (memory.voiceDatasetRefs || []) as any[]) {
      if (ref.status !== 'approved') continue;
      try {
        await TrainingDataset.updateOne(
          { name: `farmer-${memory.userId}`, version: '1.0.0', langCode: ref.langCode },
          {
            $setOnInsert: {
